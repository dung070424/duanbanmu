...
@Component({
  ...
  templateUrl: './customer-forgot-password.html',
  styleUrls: ['./customer-forgot-password.scss'],
})
export class CustomerForgotPasswordComponent implements OnDestroy {
  // Step 1: Gửi email
  email = '';
  emailError = '';
  ...
  onResetPassword() {
    ...
    console.log('🔄 [CustomerForgotPassword] Bắt đầu reset password cho email:', this.email);
    ...
    this.authService.resetPassword(
      ...
      next: (success) => {
        console.log('✅ [CustomerForgotPassword] Reset password response:', success);
        if (success) {
          ...
          setTimeout(() => {
            console.log('🔄 [CustomerForgotPassword] Redirecting to login page...');
          ...
          console.warn('⚠️ [CustomerForgotPassword] Reset password returned false');
      ...
      error: (error: any) => {
        console.error('❌ [CustomerForgotPassword] Reset password error:', error);
