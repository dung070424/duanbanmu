...
export class CounterSalesComponent implements OnInit {
  ...
  // Flag tracking validation process
  ...
  confirmPayment(): void {
    ...
    if (this.appliedCoupon) {
      ...
      this.phieuGiamGiaService.getPhieuGiamGiaByMaPhieu(code)
        .pipe(
          finalize(() => {
        ...
        .subscribe({
          next: (res: any) => {
            ...
            if (now < start) {
              ...
              this.showToast(`Phiếu giảm giá "${v.maPhieu}" chưa bắt đầu (Bắt đầu: ${startStr}).`, 'error');
              // this.removeCoupon(); // User requested to NOT remove coupon
              return;
            }
            ...
            if (now > end) {
              ...
              this.showToast(`Phiếu giảm giá "${v.maPhieu}" đã hết hạn (Hết hạn: ${endStr}).`, 'error');
              // this.removeCoupon(); // User requested to NOT remove coupon
              return;
            }
            ...
            if (v.trangThai === false) {
              this.showToast(`Phiếu giảm giá "${v.maPhieu}" đã bị vô hiệu hóa.`, 'error');
              // this.removeCoupon(); // User requested to NOT remove coupon
              return;
            }
          ...
          error: (err) => {
            ...
            // Không xóa coupon ở đây để user có thể thử lại, nhưng modal sẽ không hiện
          }
        });
    } else {
      // No voucher -> Proceed
